        ...
        <div v-if="assetSelected">
          ...
          <v-container>
            <v-row justify="space-around">
              <v-col>
                <label>Price</label>
                <v-text-field background-color="white" outlined disabled :value="price"></v-text-field>
              </v-col>
              <v-col>
...
<script>
  ...
  export default {
    ...
    computed: {
      price(){
        const asset = this.stocksData.find(stock => stock.name === this.assetSelected.name);
    ...
    methods: {
      ...
      submitTransaction() {
        ...
        AjaxCalls.makeTransaction(
          ...
          this.$store.state.ui.activePortfolio.id
        )
          .then(res => {
